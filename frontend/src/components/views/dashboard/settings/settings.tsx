...
export const Settings = ({ transition }: SettingsProps) => {
  ...
  const onChangeImageScale = (ev: any) => setImageScale(getImageScale(ev.target.value as number));

  const onChangeAspectRatio = (ev: any) => setAspectRatio(getAspectRatio(ev.target.value as number));

  const onChangeModel = (ev: any) => setModel(getModel(ev.target.value as number));

  const renderChooseImageScale = () => (
    <select id={selectImageScaleId} className="uk-select uk-margin-small-top" onChange={onChangeImageScale}>
  ...
  const renderChooseAspectRatio = () => (
    <select id={selectAspectRatioId} className="uk-select uk-margin-small-top" onChange={onChangeAspectRatio}>
  ...
  const renderChooseModel = () => (
    <select id={selectModelId} className="uk-select uk-margin-small-top" onChange={onChangeModel}>
  ...
  const submitReport = () => transition(DashboardView.REPORT, { imageScale, aspectRatio, model });
    ...
    <>
      <div>
        ...
        <div className="uk-margin-medium-top">
          ...
          <small>{"The selected multiplier will be applied to the image's width and height."}</small>
        </div>
        ...
        <div className="uk-margin-top">
          ...
          {renderChooseModel()}
          <small>{'The selected machine learning model will be applied during image processing.'}</small>
        </div>

        <button className="uk-button uk-button-primary uk-width-1-1 uk-margin-medium-top" onClick={submitReport}>
          {'Generate report'}
        </button>
